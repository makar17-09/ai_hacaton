...
descriptor_fn = {'topology': TopologyDescriptors.GetTopologyofMol, 'constitutional': ConstitutionDescriptors.GetConstitutionalofMol,
    'burden': BurdenDescriptors.GetBurdenofMol, 'basak': BasakDescriptors.GetBasakofMol, 'cats2d': Cats2dDescriptors.CATS2DforMol,
    'charge': ChargeDescriptors.GetChargeforMol, 'connectivity': ConnectivityDescriptors.GetConnectivityforMol, 'estate': EstateDescriptors._GetEstateforMol,
    'geary': GearyDescriptors.GetGearyAutoofMol, 'kappa': KappaDescriptors.GetKappaofMol, 'moe': MoeDescriptors.GetMOEofMol, 'moran': MoranDescriptors.GetMoranAutoofMol,
    'moreaubroto': MoreaubrotoDescriptors.GetMoreauBrotoAutoofMol}
...
def getAllDescriptorsforMol(mol):
    topology_descriptors = TopologyDescriptors().GetTopologyofMol(mol)
    constitution_descriptors = ConstitutionDescriptors().GetConstitutionalofMol(mol)
    burden_descriptors = BurdenDescriptors().GetBurdenofMol(mol)
    basak_descriptors = BasakDescriptors().GetBasakofMol(mol)
    cats2d_descriptors = Cats2dDescriptors().CATS2DforMol(mol)
    charge_descriptors = ChargeDescriptors().GetChargeforMol(mol)
    connectivity_descriptors = ConnectivityDescriptors().GetConnectivityforMol(mol)
    estate_descriptors = EstateDescriptors()._GetEstateforMol(mol)
    gearyauto_descriptors = GearyDescriptors().GetGearyAutoofMol(mol)
    kappa_descriptors = KappaDescriptors().GetKappaofMol(mol)
    moe_descriptors = MoeDescriptors().GetMOEofMol(mol)
    moran_descriptors = MoranDescriptors().GetMoranAutoofMol(mol)
    moreaubroto_descriptors = MoreaubrotoDescriptors().GetMoreauBrotoAutoofMol(mol)
...
class MolecularDescriptors:
    ...
    def descriptor_target_split(self, file):
        """
        Split the input data into descriptors and target DataFrames
            ...
            Returns:
                ...
                target: pandas.DataFrame
                    Target DataFrame.
        """
        target = file.loc[:, file.columns == 'Active']
        descriptors = file.loc[:, file.columns != 'Active']
        return descriptors, target


    def descriptor_target_join(self, descriptors, target):
        """
        ...
        descriptors['Active'] = target['Active']
        file = descriptors
        return file



    def getDescriptors(self, data, descriptor_type = 'topology'):
        smiles, target = self.descriptor_target_split(data)
        ...
        final_df = self.descriptor_target_join(AllDescriptors, target)
        print('\nCalculating %s descriptors completed.'%descriptor_type)
        return final_df


    def getAllDescriptors(self, data):
        smiles, target = self.descriptor_target_split(data)
        ...
        for i in range(len(smiles)):
            print('Row %d out of %d' % (i + 1, len(smiles["Smiles"])), end='')
            print('\r', end='')
            mol = Chem.MolFromSmiles(smiles["Smiles"][i])
            AllDescriptors.loc[i] = getAllDescriptorsforMol(mol)
        final_df = self.descriptor_target_join(AllDescriptors, target)
