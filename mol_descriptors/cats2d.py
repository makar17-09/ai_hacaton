...
class Cats2dDescriptors:

    def MatchAtomType(self, IndexList, AtomTypeDict):
        """
    ...
    def ContructLFromGraphSearch(self, mol):
        """
    ...
    def FormCATSLabel(self, PathLength=10):
        """
    ...
    def FormCATSDict(self, AtomDict, CATSLabel):
        """
    ...
    def AssignAtomType(self, mol):
        ...
        temp = self.ContructLFromGraphSearch(mol)
        temp.extend(res['L'])
        res.update({'L': temp})
    ...
    def CATS2DforMol(self, mol, PathLength=10, scale=3):
        """
        ...
        atomtype = self.AssignAtomType(Hmol)
        ...
        for PL in range(0, PathLength):
            ...
            temp = []
            for j in Index:
                temp.extend(self.MatchAtomType(j, atomtype))
            tempdict.update({PL: temp})

        CATSLabel = self.FormCATSLabel(PathLength)
        CATS1 = self.FormCATSDict(tempdict, CATSLabel)
    ...
    def getCATS2D(self, df_x):
        ...
        """
        r = {}
        _labels = self.FormCATSLabel()
        for key in _labels:
           r[key] = []
        for m in df_x['Smiles']:
            mol = Chem.MolFromSmiles(m)
            res = self.CATS2DforMol(mol)
            for key in _labels:
