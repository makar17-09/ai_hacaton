...
class MoranDescriptors:

    def _CalculateMoranAutocorrelation(self, mol, lag=1, propertylabel='m'):
        """
        **Internal used only**
    ...
    def CalculateMoranAutoMass(self, mol):
        """
        Calculation of Moran autocorrelation descriptors based on
        ...
        res = {}
        for i in range(8):
            res['MATSm' + str(i + 1)] = self._CalculateMoranAutocorrelation(mol, lag=i + 1, propertylabel='m')
        return res


    def CalculateMoranAutoVolume(self, mol):
        """
        Calculation of Moran autocorrelation descriptors based on
        ...
        for i in range(8):
            res['MATSv' + str(i + 1)] = self._CalculateMoranAutocorrelation(mol, lag=i + 1, propertylabel='V')

        return res


    def CalculateMoranAutoElectronegativity(self, mol):
        """
        Calculation of Moran autocorrelation descriptors based on
        ...
        res = {}
        for i in range(8):
            res['MATSe' + str(i + 1)] = self._CalculateMoranAutocorrelation(mol, lag=i + 1, propertylabel='En')
        return res


    def CalculateMoranAutoPolarizability(self, mol):
        """
        Calculation of Moran autocorrelation descriptors based on
        ...
        for i in range(8):
            res['MATSp' + str(i + 1)] = self._CalculateMoranAutocorrelation(mol, lag=i + 1, propertylabel='alapha')

        return res


    def GetMoranAutoofMol(self, mol):
        """
        ...
        """
        res = {}
        res.update(self.CalculateMoranAutoMass(mol))
        res.update(self.CalculateMoranAutoVolume(mol))
        res.update(self.CalculateMoranAutoElectronegativity(mol))
        res.update(self.CalculateMoranAutoPolarizability(mol))

        return res

    def getMoranAuto(self, df_x):
        """
        ...
        for m in df_x['Smiles']:
            ...
            res = self.GetMoranAutoofMol(mol)
            for key in labels:
